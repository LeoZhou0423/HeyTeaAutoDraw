...
brush_handler = BrushHandler()
image_processor = ImageProcessor()
coordinate_loader = CoordinateLoader()
drawer = Drawer()


def main(image_path, mode='draw'):
    """
    主函数，执行图像绘制或点击操作
    
    Args:
        image_path: 输入图像路径
        mode: 运行模式: 'draw' - 绘制图像, 'click' - 点击坐标点
    """
    try:
        print(f"\n🎨 喜茶绘图工具启动")
        print(f"📁 配置目录: {config_manager.get_config_path()}")
        print(f"🖼️  输入图像: {image_path}")
        print(f"🔧 运行模式: {mode}")
        
        # 加载画布坐标
        canvas_coordinates = coordinate_loader.load_canvas_coordinates()
        if not canvas_coordinates:
            print("❌ 无法加载画布坐标，请先运行 window_detection.py")
            return False
        
        canvas_top_left, canvas_size = canvas_coordinates
        print(f"📍 画布位置: {canvas_top_left}")
        print(f"📏 画布尺寸: {canvas_size}")
        
        if mode == 'draw':
            # 绘制模式
            print("\n🔍 正在处理图像...")
            traced_paths, stroke_widths = image_processor.extract_strict_strokes(image_path)
            print(f"📐 提取到 {len(traced_paths)} 个轮廓")
            
            # 过滤短路径
            filtered_paths = path_processor.filter_short_paths(traced_paths)
            print(f"🧹 过滤后剩余 {len(filtered_paths)} 个路径")
            
            # 检测或加载画笔滑块位置
            slider_positions = brush_handler.load_brush_slider_positions()
            if not slider_positions:
                print("🔍 正在检测画笔滑块位置...")
                slider_positions = brush_handler.detect_brush_size_slider(canvas_top_left, canvas_size)
                brush_handler.save_brush_slider_positions(slider_positions)
                print("💾 画笔滑块位置已保存")
            
            # 计算缩放因子
            # 这里需要根据实际图像和画布大小计算合适的缩放因子
            # 为了简化，我们暂时使用固定值1.0
            scale_factor = 1.0
            
            print("\n🎨 开始绘制...")
            print("💡 提示: 按空格键暂停/继续，按ESC键退出")
            
            # 执行绘制
            drawer.draw_on_canvas(filtered_paths, canvas_top_left, canvas_size, stroke_widths, scale_factor)
            
            if drawer.keyboard_handler.check_exit_condition():
                print("\n🛑 绘制已被用户中断")
            else:
                print("\n✅ 绘制完成！")
            ...
            # 加载捕获的坐标点
            captured_points = coordinate_loader.load_captured_coordinates()
            if not captured_points:
                print("❌ 没有找到捕获的坐标点")
                return False
            
            print(f"📍 找到 {len(captured_points)} 个坐标点")
            
            # 启动键盘监听器
            keyboard_handler = KeyboardHandler()
            keyboard_handler.start_listener()
            
            try:
                # 遍历所有坐标点并点击
                for i, (x, y) in enumerate(captured_points):
                    if keyboard_handler.check_exit_condition():
                        break
                    
                    print(f"点击坐标点 {i+1}: ({x}, {y})")
                    pyautogui.click(x, y)
                    time.sleep(0.1)  # 点击间隔
                    
                    # 等待直到绘制继续（如果暂停）
                    while keyboard_handler.check_pause_condition():
                        if keyboard_handler.check_exit_condition():
                            break
                        time.sleep(0.1)
            finally:
                keyboard_handler.stop_listener()
                pyautogui.mouseUp()
...
if __name__ == "__main__":
    ...
    try:
        success = main(args.image, args.mode)
        if success:
            print("\n🎉 程序执行成功！")
        else:
            print("\n⚠️  程序执行失败！")
